...
public class NewBehaviourScript : MonoBehaviour
{
    ...
    private void Update()
    {
        for (int j = 0; j < m_LayoutRebuildQueue.Count; j++)
        {
            ...
            if (ObjectValidForUpdate(rebuild) && rebuild.transform != null)
            {
                Debug.LogFormat("{0}引起{1}布局重建", rebuild.transform.name, rebuild.transform.GetComponent<Graphic>().canvas.name);
            }
        }
        ...
        for (int j = 0; j < m_GraphicRebuildQueue.Count; j++)
        {
            ...
            if (ObjectValidForUpdate(element) && element.transform != null)
            {
                Debug.LogFormat("{0}引起{1}图形重建", element.transform.name, element.transform.GetComponent<Graphic>().canvas.name);
            }
        }
    }
